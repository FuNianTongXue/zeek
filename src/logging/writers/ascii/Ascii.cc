...
void Ascii::InitConfigOptions()
	...
	json_skip_unset_fields = BifConst::LogAscii::json_skip_unset_fields;
...
bool Ascii::InitFilterOptions()
	...
	for ( WriterInfo::config_map::const_iterator i = info.config.begin(); i != info.config.end();
		...
		else if ( strcmp(i->first, "json_skip_unset_fields") == 0 )
			{
			if ( strcmp(i->second, "T") == 0 )
				json_skip_unset_fields = true;
			else if ( strcmp(i->second, "F") == 0 )
				json_skip_unset_fields = false;
			else
				{
				Error("invalid value for 'json_skip_unset_fields', must be "
...
bool Ascii::InitFormatter()
	...
	if ( use_json )
		...
		formatter = new threading::formatter::JSON(this, tf, json_skip_unset_fields);

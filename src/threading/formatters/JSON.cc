...
JSON::JSON(MsgThread* t, TimeFormat tf, bool arg_skip_unset_fields)
	: Formatter(t), surrounding_braces(true), skip_unset_fields(arg_skip_unset_fields)
	{
	timestamps = tf;
	}
...
bool JSON::Describe(ODesc* desc, int num_fields, const Field* const* fields, Value** vals) const
	...
	for ( int i = 0; i < num_fields; i++ )
		{
		if ( vals[i]->present || ! skip_unset_fields )
...
bool JSON::Describe(ODesc* desc, Value* val, const std::string& name) const
	...
	if ( (! val->present && skip_unset_fields) || name.empty() )
		return true;

...
# LogAscii::json_skip_unset_fields=F or using a filter's config table to set a
# field of the same name includes them with a null value.
#
# @TEST-EXEC: zeek -b %INPUT
# @TEST-EXEC: btest-diff testing.log
#
# @TEST-EXEC: zeek -b %INPUT LogAscii::json_skip_unset_fields=F Testing::logname=testing_nullfields
...
event zeek_init() &priority=5
{
    ...
    if ( use_config_table )
        ...
        f$config = table(["json_skip_unset_fields"] = "F");
